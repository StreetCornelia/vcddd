...
- 只从 `主控状态.md` 原样转交专业 Agent 的用户交互包和用户反馈；
- 把尚未被吸收的用户原话一次性写入 `主控状态.md`，防止会话压缩丢失；
...
Domain 分析先把业务恢复成“核心事物—拥有的信息与关系—自身行为—产生的影响”，由用户确认核心事物、所有权、关系和行为归属。系统设计 Agent 再围绕每个核心事物形成完整的 Domain：它维护什么，能做什么，依据什么判断，行为后改变和影响什么；这套 Domain 设计也由用户确认，之后才继续模块、API 和数据库。核心业务名称的含义另行确认后再扩散到 API、表和代码。只为当前已确认目标建立最小充分模型；未来可能性不能作为新增实体、版本、聚合、Domain 或基础设施的依据。
