...
用户对设计的审核不属于上述“审核 Agent”。系统设计 Agent 必须先主动完成方案，再把完整 Domain 设计、API 设计、核心接口内部编排和人类可读的数据库设计分别交给用户审核；用户不需要替 AI 逐项发明技术方案或阅读 DDL，但只有用户明确确认后，这些候选设计才成为可供下游使用的当前事实。AI 不能根据沉默、旧对话中的概括认可或“技术缺口默认由设计 Agent 解决”自行填写确认结果。
