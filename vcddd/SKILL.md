---
name: vcddd
description: "以业务设计为源头、通过具备明确认知协议的专职子 Agent 持续推进到经过增量验证、阶段审查和最终审核的可运行代码。用于从想法、文字、原型、现有系统、代码和运行证据形成由用户确认的业务、Domain、架构、模块、API、内部编排、数据库、开发基线和系统工程规范；再盘点全部业务与非业务代码产物，形成有依赖和 worktree 隔离的开发任务图，为每个任务和阶段建立真实运行验证与独立审查，最后完成跨阶段统一测试、工程改进与独立审核。适合需要跨会话恢复、多目标并行、按任务难度选择模型、持续暴露小问题并避免代码静默偏离的项目。"
---
...
正式独立审核 Agent 只对固定代码快照启动。业务设计和系统设计阶段不让另一个 AI 用“通过 / 不通过”代替用户判断。Coding 阶段的单任务审查和阶段集成审查是尽早发现实现缺陷的增量工程反馈，不重新裁决已经由用户确认的业务与设计，也不替代最终跨阶段审核。
...
任何专业阶段开始前，主控必须读取并严格使用 [references/model-routing.md](references/model-routing.md)。每个目标项目在自己的仓库中维护 `vcddd/config/agent-models.json`；本 Skill 不预置具体模型映射。

配置不存在、项目初始化时本机可访问的 Codex/Claude 环境尚未记录、当前环境没有经过本机发现、模型或推理强度已失效、或者用户尚未确认映射时，先暂停当前阶段，探测真实可用模型并形成用户确认包。不可访问的环境记录原因而不伪造模型；不能直接继承主对话模型。

默认能力档位：

- 业务、Domain、总体架构和跨系统关键判断：`deep`；
- API、内部编排、数据库、工程规范和任务图：`planning`；
- 单任务、阶段和最终代码审查：`review`；
- 边界明确的编码、测试、修复、原型和验证实现：`execution`；
- 索引、结构校验、状态回写和固定投影：`mechanical`。

主控选择满足档位的最低成本模型并显式传入模型与推理强度。只有跨事实所有权、重大歧义、不可逆风险或两次有证据的能力不足才升级；代码量大或审查条目多本身不使用最高模型。
...
- 在每个阶段派发前核对项目模型配置，显式选择档位、模型和推理强度；
...
每个无未完成前置依赖且写入范围不冲突的任务在独立 worktree 中并行实现。同批任务使用同一已提交基线；有依赖的任务只在前置代码合并后从新 Commit 开始；共享文件固定一个写入者。实施 Agent 只完成任务节点声明的生产代码、迁移、配置和装配，不自行宣布正确。任务提交以后，立即从同一固定 Commit 并行启动独立任务验证 Agent 和任务审查 Agent：前者按验证合同构建、启动或运行到声明层级，后者核对实现符合性和局部工程质量；两者通过以前不得合并或解除后续依赖。

每个任务维护 `任务进度.md`、`实现记录.md`、`任务验证.md` 和 `任务审查.md`。实施 Agent 可以运行构建、类型检查和最小自检来避免明显破损提交，但验证结论由独立 Agent 拥有；验证 Agent 可以编写本任务范围的测试和夹具但不修改生产代码；审查 Agent 只读固定任务 Commit。
...
目标是确认实现、验证和审查指向同一 Commit，实际运行层级达到合同，且实施、验证和审查 Agent 相互独立。
...
每个阶段汇合以后先形成阶段 Commit，再由独立阶段验证 Agent 对组合快照执行真实构建、启动、就绪检查和关键路径；独立阶段审查 Agent 检查跨任务接线、共享写入、错误传播和阶段范围。`阶段运行验证.md` 与 `阶段审查.md` 都通过以后，阶段才能请求用户确认或派发依赖阶段。第一处承诺用户可观察结果的阶段和最终阶段必须达到 `可用`；不能只用“编译成功”替代运行证据。
...
目标是确认阶段记录、运行验证和阶段审查指向同一阶段 Commit，并达到声明的运行层级。

全部任务和阶段按图闭合后形成统一生产代码快照和 `集成记录.md`。详细规划、实施、增量验证、阶段汇合和记录方式见 [references/coding-guidance.md](references/coding-guidance.md) 与 [references/development-agent.md](references/development-agent.md)。
...
每个任务提交后先按 [references/testing-agent.md](references/testing-agent.md) 启动独立任务验证 Agent，对该任务固定 Commit 编写或运行聚焦测试、构建和运行入口，并写入 `tasks/<TASK-ID>/任务验证.md`。验证通过只覆盖该任务声明的范围，不证明阶段或系统可用。

每个阶段 Commit 形成后启动阶段验证 Agent，对组合快照运行累计相关测试、真实启动和关键路径，并写入 `stages/<stage-id>/阶段运行验证.md`。阶段必须明确实际达到 `可构建`、`可启动`、`可运行` 或 `可用` 中哪一层。

一个开发批次的全部阶段闭合并形成唯一快照后，再启动跨阶段统一测试。主控根据实际代码和剩余风险启动多个互相独立的测试 Agent，例如 API 合同、业务结果、数据事务、外部失败恢复、并发幂等、配置启动或系统集成；不存在的对象不机械创建测试角度。统一测试复用增量测试，但必须补齐单任务和单阶段无法发现的组合风险。
...
编码完成边界仍只描述生产代码已形成；任务能否合并由独立任务验证和审查决定。测试结论为 `可进入工程改进` 后才启动工程改进。
...
每个任务 Commit 形成后立即并行调用聚焦任务审查 Agent 与任务验证 Agent；每个阶段 Commit 形成后立即并行调用阶段集成审查 Agent 与阶段运行验证 Agent。审查 Agent 使用 `review` 档位，分别写入任务和阶段目录，只检查当前固定快照与已确认输入，不等待全部生产代码形成。

计划内工程改进完成、受影响统一测试重新执行并形成最终固定生产代码与测试快照后，仍并行调用两个相互独立的核心审核：
