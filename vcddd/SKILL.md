---
name: vcddd
description: "以业务设计为源头、通过具备明确认知协议的专职子 Agent 持续推进到可审核代码的人机协作协议。用于从想法、文字、已确认原型、快速网页原型、现有系统、代码和运行证据中形成由用户确认的业务设计，再形成系统拆分、模块拆分、API、核心业务流程、数据库和开发基线，继续实现、记录业务流程与代码的对应关系，并在代码形成后由多个独立审核 Agent 检查实现符合性及工程质量。适合需要用户反复参与、跨会话恢复、多个目标或系统并行、按需制作原型或报告，以及必须避免 AI 擅自判定业务正确或代码静默偏离的项目。"
---
...
用户对设计的审核不属于上述“审核 Agent”。系统设计 Agent 必须先主动完成方案，再把完整 Domain 设计、API 设计、核心业务流程和数据库 / SQL 设计分别交给用户审核；用户不需要替 AI 逐项发明技术方案，但只有用户明确确认后，这些候选设计才成为可供下游使用的当前事实。AI 不能根据沉默、旧对话中的概括认可或“技术缺口默认由设计 Agent 解决”自行填写确认结果。
...
- `API设计.md`：精确调用契约；
- `核心业务流程.md`：按业务线说明核心流程的应用编排、Domain 调用、事务、外部协作、关键日志、失败和恢复；
...
API、核心业务流程和数据库也必须先形成可独立阅读的完整候选，再分别交给用户审核。API 确认后，系统设计 Agent 以业务线而不是接口为单位建立 `核心业务流程.md`：覆盖每条业务线，但只展开真正包含业务判断、状态变化、事务、外部协作或恢复责任的核心流程；普通列表、展示和简单查询只登记归属，不展开内部实现。流程确认后再形成数据库设计。任何一项未确认时，都不能把候选当作后续确定输入；数据库未确认时，不能生成状态为 `当前` 的开发基线或进入 Coding。用户明确要求并行时可以继续准备下游或其他系统候选，但必须清楚标记假设和待确认状态，不能绕过当前应展示的审核。只为当前已确认目标建立最小充分模型；未来可能性不能作为新增实体、版本、聚合、Domain 或基础设施的依据。
...
报告 Agent 只能摘取、压缩、排序、链接和绘制现有事实。缺少内容时返回缺失事实、应修改的源文档和负责角色；不得为了完成报告临场创造系统、Domain、模块、API、核心业务流程或数据设计。

报告投影规则见 [references/report-projection.md](references/report-projection.md)。

## 允许回到任何事实

业务设计、系统拆分、模块拆分、API、核心业务流程、数据库、开发基线、代码和审核存在通常的推导顺序，但不是门禁状态机。任何 Agent 都可以报告：
