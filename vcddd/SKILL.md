---
name: vcddd
description: "以业务设计为源头、通过主控和专职子 Agent 持续推进到可编码软件的人机协作协议。用于从想法、文字、已确认原型、快速网页原型、现有系统、代码和运行证据中形成业务设计，再为各系统形成系统拆分、模块拆分、API 设计、数据库设计和只读汇总式开发基线，继续实现、测试与反馈修正。适合需要用户反复参与、跨会话恢复、多个目标或系统并行、按需制作原型或报告，以及必须避免最终开发文档临场重新设计的项目。"
---

# VCDDD

把项目文档作为持久事实，让主控 Agent 负责用户交互、任务恢复和专业 Agent 调度。专业 Agent 只处理自己的语义空间；所有专业 Agent 通过主控通信，不依赖彼此或主会话的历史。
...
- 判断请求属于业务设计、系统与开发设计、开发、原型投影或报告投影；
...
调用时只给业务目标入口、当前任务、直接来源和 [references/business-agent.md](references/business-agent.md)。
...
设计过程中发现业务设计不足时，返回精确上游请求；不得直接修改 `business/`。调用时给当前系统入口、相关 `业务设计.md`、当前任务、代码或验证入口和 [references/system-design-agent.md](references/system-design-agent.md)。
...
代码现实与开发基线冲突时，开发 Agent 报告事实和影响，由主控路由到相应设计 Agent；代码不能静默成为新的业务或设计权威。调用时给当前任务、开发基线、本切片直接来源、仓库规范和 [references/development-agent.md](references/development-agent.md)。
...
业务设计、系统拆分、模块拆分、API、数据库、开发基线和代码存在通常的推导顺序，但不是门禁状态机。任何 Agent 都可以报告：
...
脚本只检查入口、链接、固定事实文档、开发基线来源和版本状态，不判断业务或设计是否正确。
