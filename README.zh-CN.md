...
- `steps/{STEP}/SKILL.md`：模块化子步骤（10 个），每步自洽，可独立派遣子 Agent
...
5. **TDD 桥梁**：从 business.md 机械推导测试规格——每条不变式、状态迁移、命令路径、失败分支映射到具体测试用例。
6. **任务拆分**：每个域拆分为 2-5 分钟的独立 subagent 任务，附带精确文件路径和测试 ID。
7. **Subagent 执行**：每个任务派遣独立 subagent 遵循 TDD（RED-GREEN-REFACTOR）实现。两道审查（Spec 合规 vs business.md，代码质量 vs tech-stack.md）把关每个任务。
8. **集成验证**：全部域完成后，验证跨域契约、端到端工作流、幂等和事件顺序。
9. **最终报告**：呈现域状态、测试数量和验证结果摘要。
