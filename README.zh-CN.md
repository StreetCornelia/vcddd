...
│   ├── DEVSETUP/          ← 确定技术栈
│   ├── FRONTEND-DESIGN/   ← 使用 google-design-md 完成前端设计规范
...
| **D²·⁶** | Frontend Design（前端设计指导） | 用户界面项目中，用 `google-design-md` 完成统一前端设计规范 | `docs/vcddd/frontend/*` |
...
5. **前端设计指导（如适用）**：用户界面项目先使用本地 `google-design-md` 完成统一设计规范，并写入 `docs/vcddd/frontend/`。
