...
验证以被验证系统为所有权边界。原型不是与验证并列的独立产物类型，而是 `prototype` 验证方法；技术 PoC、合同检查、源码核对、基准测试和最小端到端实现是其他验证方法。验证计划、实现代码、运行记录和结论共同保存在该系统的验证目录，不能把原型代码放在项目外的临时位置，再只在 VCDDD 中留下无法复现的结论。
...
目录中的 `index.md` 固定声明验证标识、所属系统、验证方法、命题、当前状态、当前结论入口和最近有效运行。生产代码不得导入 `validation/` 中的实现；验证代码只为形成证据服务。
...
原型源码固定写入该验证项的 `src/`，截图、录屏、用户操作观察和反馈写入具体 `runs/<RUN-ID>/`。运行记录固定声明源码 Commit、环境、入口、输入、观察、证据、适用与未覆盖范围，以及用户确认状态、依据和确认范围。一次用户确认只对记录的原型 Commit、运行编号、页面和操作范围有效；原型继续修改后不能沿用旧确认。
...
验证状态只由验证项 `index.md` 拥有，单次运行状态只由对应 `运行记录.md` 拥有。系统入口和验证总入口只生成链接与状态投影，不手工复制结论。这样修改一次验证状态后，可以由脚本同步所有导航，而不会让旧索引继续声称验证尚未开始或已经完成。
