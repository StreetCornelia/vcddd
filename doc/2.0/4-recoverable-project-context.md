...
> 状态：VCDDD 2.0 Skill 的项目工作空间与上下文恢复设计，已经根据真实 Skill 试运行修正
>
> 日期：2026-07-31
>
> 前置文档：
...
项目认知应按照使用范围分为三个逻辑层次：当前事实层、任务恢复层和证据交付层。

```mermaid
flowchart TD
    E["项目入口：当前系统、当前目标与阅读导航"]
    P["当前事实层：业务与系统的现行设计"]
    G["任务恢复层：本次要改变什么、候选判断与下一步"]
    H["证据交付层：系统验证、开发交付和演化依据"]
...
- 原型、技术 PoC 和端到端验证的计划、代码、运行与结果；
- 开发任务、阶段汇合、统一测试、工程改进和审核记录；
- 被放弃或被替代的方案及其适用条件；
- 已完成目标的详细讨论；
- 业务理解和实现发生变化的记录；
- 必须保留的失败尝试与负面经验。
...
VCDDD 不再只是 `docs/` 下的文档集合，而是项目根目录中的设计与验证工作空间。验证代码和原型代码需要与它们证明的命题放在一起，因此使用根目录 `vcddd/`；生产代码仍位于项目自己的生产代码根，不得依赖验证实现。
...
└── work/
    ...
    └── <work-id>/
        ├── index.md
        ├── 主控状态.md
        ├── Domain判定.md（进入 Domain 分析时）
        └── 语言验证/（发生表达盲测时）
```

其中只有以下入口具有稳定含义：

- `vcddd/index.md`：任何新对话进入项目时首先读取；
- `business/index.md`：阶段性业务目标及其业务设计入口；
- `systems/index.md`：系统及其当前设计、验证和交付入口；
- `systems/<system-id>/validation/index.md`：该系统全部验证项的投影；
- `systems/<system-id>/delivery/index.md`：该系统全部交付单元的投影；
- `work/index.md`：当前、等待、暂停和完成任务的投影；
- `work/<work-id>/主控状态.md`：主控与专业 Agent 的短通信面；
- `work/<work-id>/index.md`：专业 Agent 的完整恢复点。

其他文件根据实际内容产生，不能为了满足目录外观而制造空文档。目录使用稳定 ASCII 标识，中文名称保存在标题和正文中。这样既保持人类可读表达，又避免空格、编码和工具实现差异破坏脚本定位。文件仍按事实责任、业务问题或稳定关注点拆分，不按图的技术类型机械拆分。
...
`vcddd/index.md` 是新 AI 唯一必须无条件读取的项目认知文件。它不承担完整业务说明，而是回答：现在是什么项目、当前做什么、从哪里继续、什么材料与本次问题有关。
...
3. 更新唯一状态拥有者；索引和上层摘要通过同步脚本重新生成。

如果同一变化影响多个投影，AI 需要同步检查相关图、文字、决策、验证说明、设计和代码指导，避免只修改其中一个事实拥有者。索引不得手工复制这些结论。

### 12.1 单一状态源与生成索引

同一个状态只能有一个可写拥有者：

- 任务通信状态由 `work/<work-id>/主控状态.md` 拥有；
- 实施任务状态由 `delivery/<delivery-id>/tasks/<TASK-ID>/任务进度.md` 拥有；
- 阶段汇合、阶段 Commit 和用户门禁由 `stages/<stage-id>/阶段记录.md` 拥有；
- 验证状态由 `validation/<VAL-ID>/index.md` 拥有；
- 最终统一集成、测试、改进和审核状态分别由各自记录拥有。

全局、业务、系统、验证、交付和工作索引只在受控生成区中保存链接与状态投影。脚本以状态拥有者为输入更新生成区，并提供只读检查模式；发现差异时校验失败，不能让 Agent 从多份相互冲突的摘要中猜测哪份有效。

### 12.2 可脚本化子节点与语义边界

凡是输入、输出和判定条件都能由仓库事实唯一决定的子节点，应优先交给脚本：

- 从唯一状态拥有者生成全局、业务、系统、验证、交付和工作索引；
- 检查固定目录、稳定标识、必需文件、链接、字段和状态枚举；
- 检查验证项是否保存计划、结论、实现和运行记录，原型是否位于验证项 `src/`；
- 检查已派发任务的进度、实现、任务验证与任务审查记录，阶段记录、阶段运行验证与阶段审查记录，以及最终集成记录是否处于正确层级；
- 检查确认记录是否具备可恢复依据、生产代码是否已经纳入 Git，以及任务图声明的固定输入是否存在。

脚本不能决定业务是否完整、Domain 是否合理、验证命题是否被证据支持、两个 Commit 是否在业务意义上等价、代码是否正确或用户是否真的确认。它只能检查这些判断是否由唯一拥有者以规定形式记录。Agent 先完成语义判断并写入状态拥有者，再调用脚本同步和校验；不得让脚本根据文件存在或阶段名称自动创造“已确认”“已完成”结论。

### 12.3 工具必须出现在对应流程节点

仅把脚本放进 `scripts/` 不能形成可执行协议。每个需要机械处理的流程节点必须同时说明：

- 使用哪个工具及完整命令；
- `<skill-root>`、`<repo-root>`、`<system-id>`、`<delivery-id>` 或 `<work-id>` 分别指什么；
- 工具读取什么、写入什么以及不会做什么；
- 当前调用目标和通过以后允许继续的动作；
- 失败后应修改哪个状态拥有者或合同。

迁移旧目录由迁移 Agent 按目录合同完成，脚本只在迁移后同步索引和检查结构；设计候选在交给用户前使用对应系统参数；Coding 候选和实际 worktree 准入使用不同参数；系统验证更新使用基础检查；完整交付记录形成后才使用审核批次检查。普通 Agent 执行脚本，不把脚本源码当作权威合同完整读取；只有维护脚本或脚本行为与文档冲突时才读取最小相关区段。
...
11. 原型是系统验证方法，源码、运行产物和结论都保存在该系统的验证项中；
12. 状态只在一个拥有者中写入，所有索引都由脚本生成或检查，不能手工复制状态。
